...
const fetchProducts = async (
  ...
  return await getCollectionProducts("", slug || "", {
    page: page,
...
const Product = ({ product }: { product: any }) => {
  ...
  const getMrp = (product: any) => {
    ...
    return (
      <span className="inline-block text-gray-600 line-through text-xs">
    ...
    return (
      <div className="flex">
        ...
        <span className="text-black text-lg">
          ₹{Math.ceil(product.price - commission)}
        </span>
      </div>
    );
    // }
...
function filterDialogMobile(
  ...
  return (
    <div className="  inset-0 bg-white mt-[4px] h-3/4  z-50 flex flex-col  text-black w-full">
      <div className="flex px-4 flex-grow justify-between text-black h-[20px]">
        <span>Filters</span>
        <XMarkIcon
          strokeWidth={2.5}
      ...
      <div className="p-4 bg-gray-300 sticky bottom-0">
        <div className="flex justify-center gap-5">
          <button
            ...
            className="rounded-full border border-[#6a4fa3] uppercase text-[#6a4fa3]  px-4 text-sm disabled:bg-opacity-50"
            onClick={() => {
              setSelectedTypes([]);
              setProductTypes([]);
            }}
  ...
  return (
    <section className="md:w-3/4">
      {result?.loading || filterChange ? (
        ...
        <>
          <div className="hidden md:visible md:flex md:justify-between pb-2 pt-3">
    ...
    <div
      className={`${
        showFilter && isMobile
          ...
          : "hidden"
      }  md:block border md:w-2/3 lg:w-1/4 accent-purple-800 bg-[#F9F9FC] h-max px-4 md:px-10 py-3 border-separate rounded-lg shadow-lg my-2 mb-8`}
    >
      <div>
